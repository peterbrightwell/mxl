...
TEST_CASE_PERSISTENT_FIXTURE(mxl::tests::mxlDomainFixture, "Fabrics: Target batch grain drain", "[Fabrics][Transfer][Flows][Grain][Batch]")
{
    auto instance = mxlCreateInstance(domain.c_str(), "");
    ...
    // Invalid arguments are rejected without needing a connection.
    {
        std::uint64_t badIndices[4];
        std::size_t badCount = 123;
        REQUIRE(mxlFabricsTargetReadGrainsNonBlocking(nullptr, badIndices, 4, &badCount) == MXL_ERR_INVALID_ARG);
        REQUIRE(mxlFabricsTargetReadGrainsNonBlocking(target, nullptr, 4, &badCount) == MXL_ERR_INVALID_ARG);
        REQUIRE(mxlFabricsTargetReadGrainsNonBlocking(target, badIndices, 0, &badCount) == MXL_ERR_INVALID_ARG);
        REQUIRE(mxlFabricsTargetReadGrainsNonBlocking(target, badIndices, 4, nullptr) == MXL_ERR_INVALID_ARG);
    }

    REQUIRE(mxlFabricsInitiatorAddTarget(initiator, targetInfo) == MXL_STATUS_OK);

    std::uint64_t indices[8];
    std::size_t count = 0;

    // The batch API also drives the connection forward, so it can be used during connection establishment too.
    waitForConnection([&]() { mxlFabricsTargetReadGrainsNonBlocking(target, indices, 8, &count); },
        [&]() { return mxlFabricsInitiatorMakeProgressNonBlocking(initiator); });

    waitForTransferStart([&]() { mxlFabricsTargetReadGrainsNonBlocking(target, indices, 8, &count); },
        [&]() { requireProgressStatus([&]() { return mxlFabricsInitiatorMakeProgressNonBlocking(initiator); }, "initiator"); },
        [&]() { return mxlFabricsInitiatorTransferGrain(initiator, 0, 0, 1); });

    // Drain the completion(s) for the transferred grain with the batch API and assert it reports them.
    pollUntilSuccess(
        [&]()
        {
            requireProgressStatus([&]() { return mxlFabricsInitiatorMakeProgressNonBlocking(initiator); }, "initiator");
            count = 0;
            auto const status = mxlFabricsTargetReadGrainsNonBlocking(target, indices, 8, &count);
            if (status != MXL_STATUS_OK && status != MXL_ERR_NOT_READY)
            {
                FAIL("Batch drain failed: " + std::to_string(status));
            }
            return status == MXL_STATUS_OK && count >= 1;
        },
        POLL_TIMEOUT,
        "Batch drain did not yield the transferred grain in 5 seconds");
